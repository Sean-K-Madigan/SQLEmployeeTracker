...
const queries = {
  ...
  //   const department_id = pool.query('SELECT id FROM departments WHERE name = $1', [department]);
  //   console.log(department_id.rows[0].id);
  //   return pool.query('DELETE FROM departments WHERE name = $1', [department_id.rows[0].id]);
  // },
